...
export function setCookie(key: string, value: string, domain?: string, expire?: number) {
    let expires = '';
    let date = new Date();
    let days = expire || 30;
    ...
    let cookie = domain ? `${key}=${encodeURIComponent(value)}${expires}; path=/; domain=${domain}` : `${key}=${encodeURIComponent(value)}${expires}; path=/`;
...
export function setCookieWithout(key: string, value: string, path?: string) {
    ...
    document.cookie = `${key}=${encodeURIComponent(value)};path=${path ? path : '/'}`;
}
